...
public class VideoMapper {
    ...
    public List<Map<String,Object>> selectWhoGiveMeLike(){
        return jdbcTemplate.queryForList("SELECT user_id FROM video_like");
    }
    ...
    //删除某视频的全部观看历史（配合删除视频，避免外键冲突）
    public int deleteHistoryByVideo(long videoId) {
        return jdbcTemplate.update("DELETE FROM video_history WHERE video_id=?", videoId);
    }
    ...
    public List<Map<String, Object>> selectVideoByLikes() {
        return jdbcTemplate.queryForList(SELECT_WITH_UPLOADER + " ORDER BY like_count DESC");
    }
    //查看观看历史
    public List<Map<String,Object>> selectHistoricalVideo(long userId){
        return jdbcTemplate.queryForList("""
            SELECT v.id, v.title, v.video_path, v.user_id AS uploader_id,
                   h.progress, h.last_view_time
            FROM video_history h
            JOIN video v ON v.id = h.video_id
            WHERE h.user_id = ?
            ORDER BY h.last_view_time DESC
            LIMIT 20;
        """,userId);
    }
    //记录/更新观看进度，同一用户同一视频只保留一条，重复观看刷新时间和进度
    public int recordHistory(long userId,long videoId,int progress) {
        return jdbcTemplate.update("""
                INSERT INTO video_history(user_id, video_id, progress)
                VALUES (?, ?, ?)
